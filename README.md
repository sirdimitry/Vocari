...
- Окно намеренно без рамки и крестика — так OBS Window Capture захватывает только аватар, без title bar и границ. Управление — через иконку в системном трее (значок "V"):
  ...
  - "Настройки…" — окно настроек с вкладками:
    - "Рендер" — тумблер GPU (RTX) / CPU (пока только сохраняет выбор, сам GPU-бэкенд впереди) и тумблер "Покачивание" (ахоге + подпрыгивание тела при речи — последнее заработает на Этапе 3 вместе с TTS);
    ...
    - остальные разделы (TTS/Twitch/Тест) появятся на Этапе 4.
...
vocari/
  ...
  tts/        — (Этап 3) провайдеры синтеза речи
  twitch/     — (Этап 5) источник команд чата
  ui/         — трей, окно настроек, окно лога
assets/
