...
Модели аватаров и две иконки приложения в сборку через PyInstaller не встраиваются — копируются рядом обычными файлами, чтобы импорт своих моделей (Настройки → Модель) мог дописывать туда новые папки, как при запуске из исходников. Баннер GitHub используется только в документации и не входит в приложение и установщик.

При каждом запуске скачанные языковые модели Silero находятся в новом и старом расположении кеша, проверяются и автоматически загружаются с диска. Эта проверка не скачивает файлы повторно: для повреждённой модели показывается ошибка и кнопка повторного скачивания. Piper проверяет все голоса каталога и различает отсутствующие, неполные и повреждённые файлы. Уже существующий движок, требующий обновления, отображается отдельно от отсутствующего.
